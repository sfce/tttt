 ...
 * Time: 下午8:03
 */
public class WhiteWordFragment extends ListFragment {

    private WhiteWordDao dao;
    private List<WhiteWord> whiteWords;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        empty.setText("包含展示在这里的关键字的短信将被放行");
        DaoMaster.DevOpenHelper dbOpenHelper = new DaoMaster.DevOpenHelper(getActivity(), MyApplication.DB_NAME, null);
        SQLiteDatabase db = dbOpenHelper.getReadableDatabase();
        DaoMaster daoMaster = new DaoMaster(db);
        dao = daoMaster.newSession().getWhiteWordDao();
        whiteWords = dao.loadAll();
        setListAdapter(new ArrayAdapter<WhiteWord>(getActivity(), android.R.layout.simple_list_item_1, whiteWords));
    }
